...
Aprendo direttamente `index.html` (`file://`), l'app ripristina il database attivo da IndexedDB. Se non esiste una copia locale, mostra un planner vuoto e non carica la DEMO. Il pulsante **Apri database** importa il JSON selezionato in IndexedDB; **Nuovo**, dopo una conferma esplicita, sostituisce la copia attiva con un database vuoto.
